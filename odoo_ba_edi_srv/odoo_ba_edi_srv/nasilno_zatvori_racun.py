...
COUNT_FAIL=60
@DBOS.step()
def fprint_nasilno_zatvori_racun():


  content="301,1,______,_,__;"
  file_name = ""
  ...
  with tempfile.NamedTemporaryFile(dir=OUT_DIR, suffix=".txt", delete=False) as f:
    ...
    with engine.begin() as sql_session:
      query = fiscal_log.insert().values(input=content, input_file=file_name, type='NONFISCAL', input_number='NZ_RN')
      ...
      stmt = stmt.where(fiscal_log.c.input_number == 'NZ_RN')
...
@DBOS.step()
def fprint_nasilno_zatvori_racun_response(id: UUID):
    ...
    if count >= COUNT_FAIL:
      ...
      with open(answer_file) as af:
        ...
        for line in af:
            ...
            m = re.match(r"301,1,\d{6},\d+,(\w+);", line)
