...
COUNT_FAIL=60
@DBOS.step()
def fprint_polog(iznos: float = 100):

  s_iznos_polog = "{:10.2f}".format(iznos).strip()

  content = f"70,1,______,_,__;+{s_iznos_polog}"
  file_name = ""
  ...
  with tempfile.NamedTemporaryFile(dir=OUT_DIR, suffix=".txt", delete=False) as f:
    ...
    with engine.begin() as sql_session:
      query = fiscal_log.insert().values(input=content, input_file=file_name, type='NONFISCAL', input_number='POLOG')
      ...
      stmt = stmt.where(fiscal_log.c.input_number == 'POLOG')
...
@DBOS.step()
def fprint_polog_response(id: UUID):
    ...
    if count >= COUNT_FAIL:
      ...
      with open(answer_file) as af:
        ...
        for line in af:
            ...
            m = re.match(r"70,1,\d{6},\d+,(\w+);", line)
