...
COUNT_FAIL=60
@DBOS.step()
def fprint_periodicni_izvjestaj(dDatOd: datetime.date, dDatDo: datetime.date):

  #79,1,______,_,__;010125;311225;;;

  s_od = dDatOd.strftime("%d%m%y")
  s_do = dDatDo.strftime("%d%m%y")
  
  content=f"79,1,______,_,__;{s_od};{s_do};;;"
  file_name = ""
  ...
  with tempfile.NamedTemporaryFile(dir=OUT_DIR, suffix=".txt", delete=False) as f:
    ...
    with engine.begin() as sql_session:
      query = fiscal_log.insert().values(input=content, input_file=file_name, type='REPORT', input_number='PERIOD')
      ...
      stmt = stmt.where(fiscal_log.c.input_number == 'PERIOD')
...
@DBOS.step()
def fprint_periodicni_izvjestaj_response(id: UUID):
    ...
    if count >= COUNT_FAIL:
      ...
      with open(answer_file) as af:
        ...
        for line in af:
            ...
            m = re.match(r"79,1,\d{6},\d+,(\w+);.*", line)
