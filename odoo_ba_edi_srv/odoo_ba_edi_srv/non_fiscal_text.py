...
@DBOS.step()
def fprint_non_fiscal(txt: str):

  content=f"""38,1,______,_,__;
42,1,______,_,__;{txt}
39,1,______,_,__;          
"""
  file_name = ""
  ...
  with tempfile.NamedTemporaryFile(dir=OUT_DIR, suffix=".txt", delete=False) as f:
    file_name = f.name
    logging.info(f"file: {file_name}; text: {txt}")
    ...
    with engine.begin() as sql_session:
      query = fiscal_log.insert().values(input=content, input_file=file_name, type='NONFISCAL')
      ...
      #print( result[0])
      #print( result._mapping['id'])

      return (result.first())._mapping["id"]


@DBOS.step()
def fprint_non_fiscal_response(id: UUID):
...
#C:\fiscal>type answer\tmpbfq9p0wi.txt
#38,1,000200,1,Ok;
#42,1,000200,2,Ok;ErnadHusremoviŠ
#39,1,000200,3,Ok;