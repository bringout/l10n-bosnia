...
COUNT_FAIL=60
@DBOS.step()
def fprint_dnevni_izvjestaj():

  # "N" - bez ciscenja prodaje; "A" - sa ciscenjem prodaje
  content="69,1,______,_,__;0;A;"
  file_name = ""
  ...
  with tempfile.NamedTemporaryFile(dir=OUT_DIR, suffix=".txt", delete=False) as f:
    ...
    with engine.begin() as sql_session:
      query = fiscal_log.insert().values(input=content, input_file=file_name, type='REPORT', input_number='DNEVNI')
      ...
      stmt = stmt.where(fiscal_log.c.input_number == 'DNEVNI')
...
@DBOS.step()
def fprint_dnevni_izvjestaj_response(id: UUID):
    ...
    if count >= COUNT_FAIL:
      ...
      with open(answer_file) as af:
        ...
        for line in af:
            ...
            m = re.match(r"69,1,\d{6},\d+,(\w+);0;A;", line)
