...
def fprint_nuliraj_racun():
  ...
  with tempfile.NamedTemporaryFile(dir=OUT_DIR, suffix=".txt", delete=False) as f:
    ...
    with engine.begin() as sql_session:
      query = fiscal_log.insert().values(input=content, input_file=file_name, type='NONFISCAL', input_number='NUL_RN')
      ...
      stmt = stmt.where(fiscal_log.c.input_number == 'NUL_RN')
...
@DBOS.step()
def fprint_nuliraj_racun_response(id: UUID):
